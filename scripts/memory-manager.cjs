...
const WIKI_DIR = path.join(__dirname, '..', '.ai-factory', 'wiki');
...
class MemoryManager {
  ...
  // === INTEGRAÇÃO COM WIKI PERSISTENTE ===

  // Mapeia tipos de memória para diretórios do wiki
  _getWikiDirForType(type) {
    const typeMap = {
      'decision': 'decisions',
      'adr': 'decisions',
      'concept': 'concepts',
    ...
    return typeMap[type] || 'concepts';
  }

  // Gera nome de arquivo seguro para wiki
  _generateWikiFilename(content, metadata) {
    const timestamp = new Date().toISOString().slice(0, 10);
    const agent = metadata.agent || 'unknown';
    const hash = crypto.createHash('sha256').update(content.slice(0, 100)).digest('hex').slice(0, 8);
    const slug = content
      .slice(0, 50)
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
    return `${timestamp}-${agent}-${slug}-${hash}.md`;
  }

  // Salva memória sincronizada no wiki persistente
  async saveMemoryWithWiki(content, metadata = {}) {
    // Salva no SQLite
    const sqliteResult = await this.saveMemory(content, metadata);
    
    // Salva no wiki persistente
    const wikiDir = path.join(WIKI_DIR, this._getWikiDirForType(metadata.type || 'general'));
    ...
    const filename = this._generateWikiFilename(content, metadata);
    const filepath = path.join(wikiDir, filename);
    
    // Formata conteúdo em Markdown
    const wikiContent = this._formatWikiContent(content, metadata);
    fs.writeFileSync(filepath, wikiContent, 'utf-8');
    
    // Atualiza index do diretório se existir
    await this._updateWikiIndex(wikiDir);
    
    // Log de sessão (append-only)
    await this._appendToSessionLog({
      action: 'memory_saved',
      sqliteId: sqliteResult.id,
      wikiFile: filename,
      metadata,
  ...
  // Formata conteúdo para Markdown do wiki
  _formatWikiContent(content, metadata) {
    const timestamp = new Date().toISOString();
    const tags = metadata.tags ? metadata.tags.join(', ') : 'none';
    
    return `---
title: "${metadata.type || 'Memory'} - ${timestamp.slice(0, 10)}"
...
# ${metadata.type || 'Memory'} - ${metadata.agent || 'Unknown Agent'}

**Data:** ${timestamp}  
**Agente:** ${metadata.agent || 'Unknown'}  
**Tipo:** ${metadata.type || 'general'}  
**Tags:** ${tags}
...
*Gerado automaticamente por memory-manager.cjs*
`;
  }

  // Atualiza index do diretório do wiki
  async _updateWikiIndex(wikiDir) {
    ...
    let indexContent = `# Index - ${path.basename(wikiDir)}\n\n`;
    indexContent += `**Atualizado:** ${new Date().toISOString()}\n\n`;
    indexContent += `**Total:** ${files.length} documentos\n\n`;
    indexContent += '---\n\n';
    
    for (const file of files.slice(0, 50)) { // Limite de 50 no index
      const filepath = path.join(wikiDir, file);
      const stats = fs.statSync(filepath);
      const firstLine = fs.readFileSync(filepath, 'utf-8').split('\n')[0];
      
      indexContent += `- [${file}](./${file}) - ${firstLine.slice(0, 100)} (${stats.size} bytes, ${stats.mtime.toISOString().slice(0, 10)})\n`;
    }
    
    fs.writeFileSync(indexFile, indexContent, 'utf-8');
  }

  // Adiciona entrada ao log de sessão (append-only)
  async _appendToSessionLog(entry) {
    const sessionDir = path.join(WIKI_DIR, 'session');
    if (!fs.existsSync(sessionDir)) {
      fs.mkdirSync(sessionDir, { recursive: true });
    }
    
    const logFile = path.join(sessionDir, 'log.md');
    const logEntry = `\n## [${entry.timestamp}] ${entry.action}\n\n${JSON.stringify(entry, null, 2)}\n`;
    
    fs.appendFileSync(logFile, logEntry, 'utf-8');
  }

  // Busca memórias no wiki persistente
  async searchWiki(query, options = {}) {
    const { dir = null, limit = 10 } = options;
    const searchDir = dir ? path.join(WIKI_DIR, dir) : WIKI_DIR;
    ...
    const results = [];
    const files = this._getAllMarkdownFiles(searchDir);
    const queryLower = query.toLowerCase();
    
    for (const file of files.slice(0, 100)) { // Limite de busca
      try {
        const content = fs.readFileSync(file, 'utf-8');
        const score = this._calculateWikiScore(content, queryLower);
        
        if (score > 0) {
          results.push({
            file: path.relative(WIKI_DIR, file),
            path: file,
            score,
  ...
  // Recursivamente busca todos os arquivos Markdown
  ...
  // Calcula score de relevância para busca no wiki
  _calculateWikiScore(content, query) {
    let score = 0;
    ...
    // Boost para matches no título/frontmatter
    const firstLines = content.slice(0, 500);
    const firstMatches = firstLines.match(new RegExp(queryTerms.join('|'), 'gi'));
    if (firstMatches) {
      score += firstMatches.length * 10;
    }
    
    return score;
  }

  // Busca híbrida: SQLite + Wiki
  async searchHybrid(query, options = {}) {
    const { topK = 10, useWiki = true, useSqlite = true } = options;
    const results = {
      sqlite: [],
      wiki: [],
      merged: []
    ...
    if (useSqlite) {
      results.sqlite = await this.searchMemories(query, topK, options.filters || {});
    }
    
    if (useWiki) {
      results.wiki = await this.searchWiki(query, { limit: topK });
    }
    
    // Merge dos resultados
    results.merged = [
      ...results.sqlite.map(r => ({ ...r, source: 'sqlite' })),
      ...results.wiki.map(r => ({ ...r, source: 'wiki' }))
    ].sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, topK);
    
    return results;
  }

  // Exporta memórias do SQLite para o Wiki
  async exportToWiki(options = {}) {
    const { type = null, agent = null, days = null } = options;
    const filters = {};
    if (type) filters.type = type;
    if (agent) filters.agent = agent;
    if (days) filters.days = days;
    
    const memories = await this.searchMemories('*', 1000, filters);
    const exported = [];
    
    for (const memory of memories) {
      const wikiResult = await this.saveMemoryWithWiki(
        memory.content,
        { ...memory.metadata, exportedFrom: 'sqlite' }
      );
      exported.push(wikiResult);
    }
    
    return {
      total: memories.length,
      exported: exported.length,
      files: exported.map(e => e.wikiFile)
    };
  }

  // Sincroniza wiki com SQLite (deduplicação)
  async syncWikiWithSqlite() {
    const wikiFiles = this._getAllMarkdownFiles(WIKI_DIR);
    let synced = 0;
    let skipped = 0;
    ...
    for (const file of wikiFiles) {
      try {
        ...
        if (frontmatterMatch) {
          try {
            // Parse simples de frontmatter YAML-like
        ...
        if (!exists) {
          await this.saveMemory(body, { ...metadata, wikiFile: file });
          synced++;
        } else {
          skipped++;
        }
    ...
    return { synced, skipped, total: wikiFiles.length };
  }
}
...
Comandos:
  ...
  wiki-save <texto>         Salva memória sincronizada com wiki persistente
    --agent <nome>            Nome do agente
    --type <tipo>             Tipo da memória
    --tags <tag1,tag2>        Tags

  wiki-search <query>       Busca no wiki persistente
    --dir <diretório>         Diretório específico (entities, concepts, decisions)
    --limit <n>               Limite de resultados

  hybrid-search <query>     Busca híbrida: SQLite + Wiki
    --topK <n>                Quantidade de resultados
    --sqlite                  Apenas SQLite
    --wiki                    Apenas Wiki

  export-to-wiki            Exporta memórias do SQLite para wiki
    --type <tipo>             Filtrar por tipo
    --agent <nome>            Filtrar por agente
    --days <n>                Últimos N dias

  sync                      Sincroniza wiki com SQLite (deduplicação)
  ...
  try {
    switch (cmd) {
      ...
      case 'wiki-save': {
        ...
        const tagsStr = _getArg(args, '--tags') || '';
        const tags = tagsStr ? tagsStr.split(',').map(t => t.trim()) : [];
        const result = await mm.saveMemoryWithWiki(content, { agent, type, tags });
        console.log(JSON.stringify({ status: 'ok', ...result, agent, type, tags }, null, 2));
        break;
      }
      ...
      case 'wiki-search': {
        const query = args[1];
        if (!query) { console.error('Erro: forneça a query de busca'); process.exit(1); }
        const dir = _getArg(args, '--dir');
        const limit = parseInt(_getArg(args, '--limit') || '10', 10);
        const results = await mm.searchWiki(query, { dir, limit });
        console.log(JSON.stringify({ status: 'ok', query, count: results.length, results }, null, 2));
        break;
      }
      ...
      case 'hybrid-search': {
        const query = args[1];
        if (!query) { console.error('Erro: forneça a query de busca'); process.exit(1); }
        const topK = parseInt(_getArg(args, '--topK') || '10', 10);
        const useSqlite = !args.includes('--wiki');
        const useWiki = !args.includes('--sqlite');
        const results = await mm.searchHybrid(query, { topK, useSqlite, useWiki });
        console.log(JSON.stringify({ status: 'ok', query, results }, null, 2));
        break;
      }
      ...
      case 'export-to-wiki': {
        ...
        const agent = _getArg(args, '--agent');
        const days = _getArg(args, '--days');
        const result = await mm.exportToWiki({ type, agent, days: days ? parseInt(days, 10) : null });
        console.log(JSON.stringify({ status: 'ok', ...result }, null, 2));
        break;
      }

      case 'sync': {
        const result = await mm.syncWikiWithSqlite();
        console.log(JSON.stringify({ status: 'ok', ...result }, null, 2));
        break;
      }
